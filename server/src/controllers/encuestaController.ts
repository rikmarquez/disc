...
export const obtenerPreguntasE9 = async (req: Request, res: Response): Promise<void> => {
  try {
    const preguntasE9 = await prisma.preguntaE9.findMany({
      orderBy: { grupo: 'asc' },
      select: {
        grupo: true,
        opcion: true,
...
export const guardarRespuestaE9 = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const respuestaE9 = await prisma.respuestaE9.upsert({
      where: {
        encuestadoId_grupo: {
          encuestadoId: encuestado.id,
          grupo,
    ...
    // Contar progreso E9
    const totalRespuestasE9 = await prisma.respuestaE9.count({
...
const calcularEneatipo = (opcionGrupo1: string, opcionGrupo2: string) => {
  const MATRIZ_E9: { [key: string]: { numero: number; nombre: string } } = {
...
export const finalizarEncuesta = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const respuestasE9 = await prisma.respuestaE9.findMany({
      where: { encuestadoId: encuestado.id },
      orderBy: { grupo: 'asc' },
    });
